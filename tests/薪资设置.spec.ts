...
test("编辑校验", async ({ page }) => {
  await test.step("基础", async () => {
    ...
    await expect(page.getByTestId("salary-input")).toHaveValue("10");
    await expect(page.getByTestId("cannot-empty-alert")).not.toBeVisible();
    await expect(page.getByTestId("save-button")).toBeDisabled();
  ...
  await test.step("其他", async () => {
    await page.goto(xinzi_url + "/edit/ara31u88462y859"); // 三轮车司机
    await expect(page.getByTestId("salary-input")).toHaveValue("28");
    await expect(page.getByTestId("work-select")).toContainText("三轮车司机");
    await expect(page.getByTestId("cannot-empty-alert")).not.toBeVisible();
    await expect(page.getByTestId("warning-alert")).toBeVisible();
    await expect(page.getByTestId("save-button")).toBeDisabled();
    await page.getByTestId("salary-input").hover();
    ...
    await expect(page.getByTestId("cannot-empty-alert")).toBeVisible();
    ...
    await expect(page.getByTestId("duplicate-work-alert")).toContainText(
      "当前记录与 无工人 三轮车司机 工种 重复"
    );
    await expect(page.getByTestId("save-button")).toBeDisabled();
    ...
    await page.getByTitle("张孝刚").click();
    await page.locator(".anticon.anticon-close-circle > svg").nth(1).click();
    await expect(page.getByTestId("duplicate-worker-alert")).toContainText(
      "当前记录与 张孝刚 工人（无工种）重复"
    );
    await expect(page.getByTestId("save-button")).toBeDisabled();
    await page.getByTestId("work-select").click();
    await page.getByTitle("重工").click();
    await expect(page.getByTestId("duplicate-alert")).toContainText("当前记录与 张孝刚 工人 和 重工 工种 重复");
    await expect(page.getByTestId("save-button")).toBeDisabled();
  });
});
test.describe("添加编辑删除", () => {
  ...
  test("添加编辑删除", async ({ page }) => {
    ...
    await page.getByTestId("worker-select").click();
    await page.getByTitle("杨秀梅").click();
    await page.getByTestId("work-select").click();
    await page.getByTitle("重工").click();
    await page.getByTestId("salary-input").hover();
    await page.getByRole("button", { name: "Increase Value" }).click();
    await page.getByTestId("save-button").click();
    await expect(page.getByTestId("row-worker-3")).toContainText("杨秀梅");
